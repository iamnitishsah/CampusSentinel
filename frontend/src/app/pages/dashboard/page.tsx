...
// Updated Alert interfaces to match the new API response
interface AlertDetails {
    // Fields for entity-based alerts
    entity_id?: string;
    name?: string;
    role?: string;
    location?: string;
    ...
    // Fields for location-based alerts
    ...
    const fetchAlerts = useCallback(async () => {
        try {
            ...
            if (res.ok) {
                ...
                setAlertsCount(data.count); // Set the count from the API response
            } else {
                console.error("Failed to fetch alerts:", res.statusText);
                setAlerts([]);
                setAlertsCount(0); // Reset count on failure
            }
        } catch (err) {
            console.error("Error fetching alerts:", err);
            setAlerts([]);
            setAlertsCount(0); // Reset count on error
        }
    }, []);
                                    ...
                                    <div className="flex items-start justify-between mb-2">
                                        <div className="flex-1">
                                            ...
                                            {alert.details.name ? (
                                                ...
                                                <h3 className="font-semibold text-white">
                                                    {alert.details.location_name || alert.alert_type}
                                                </h3>
                                            )}
                                        </div>
